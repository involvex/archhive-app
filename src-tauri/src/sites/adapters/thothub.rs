...
const BASE: &str = "https://thothub.to";
...
impl SiteAdapter for ThotHubAdapter {
    ...
    async fn browse(&self, ctx: &SiteContext, query: BrowseQuery) -> AppResult<BrowsePage> {
        ...
        let mut items = parse_listing(&html, self.id());
...
fn build_browse_url(query: &BrowseQuery) -> AppResult<String> {
    ...
    let path = match query.kind {
        ...
        BrowseKind::Tag => format!("/tags/{}/", path_slug(&query.slug)),

        BrowseKind::Model => format!("/model/{}/", path_slug(&query.slug)),
        ...
        BrowseKind::Video => {
            ...
            format!("/video/{}/", path_slug(&query.slug))
...
fn parse_listing(html: &str, site_id: &str) -> Vec<MediaItem> {
    ...
    for sel_str in container_selectors {
        ...
        let Ok(container_sel) = Selector::parse(sel_str) else { continue };
...
mod tests {
    ...
    fn build_model_url() {
        ...
        assert!(build_browse_url(&q).unwrap().contains("/model/sweetie-fox/"));
