...
> Numbering note: `41–43` below reuse numbers from the earlier Library section for historical reasons. New Android items continue at `55+` and new quick wins at `Q39+`.
...
### 55. Foreground-Service Download Notifications + Channels

**Status:** 🔵 Partial  
**Area:** Mobile / Downloads  
**Currently:** Native completion/failed toasts via `tauri-plugin-notification` (#11) + in-app `react-hot-toast`; mobile `DownloadProgressRow` shows in-app progress only; no ongoing Android progress notification or notification-channel settings.  
**Done:** Android channels registered at startup (mobile-only `register_download_channels` in `downloads/manager.rs`: `archhive-downloads-progress` Low, `archhive-downloads-done` Default, `archhive-downloads-error` High); completion + failure notifications tagged with their channel via `.channel_id()` (desktop ignores the id).  
**Suggestion (remaining):**

- Ongoing progress notification per active download (percent + pause/cancel actions) via foreground service / Android notification API; grouped summary for batch queues. Needs native Kotlin wiring in `gen/android` (outside Tauri IPC) — treat as stretch.
- Notification channels: Downloads-progress (low importance, silent), Downloads-done (default), Errors (high); user-toggleable in system settings via Settings → Notifications deep link (#43).
- Tap notification → opens Downloads page; respects #45 Wi-Fi-only / battery-saver paused state ("Waiting for Wi-Fi" text).
- Desktop keeps existing tray + toast behavior — same labels, no foreground service.
...
| Q34 | **Wi-Fi-only download guard**       | Warn/block queueing downloads on mobile data; "Wi-Fi only" setting in Downloads (default ON on mobile).                                                         | ✅ Done (via #45: `download_on_wifi_only` default ON on mobile, `waiting_for_wifi` job status, cyan label + Retry now, `NetworkMonitor` + `useNetworkMonitor`) |
...
| Q37 | **Thumb quality toggle (mobile)**   | Low/Med/High sidecar JPEG quality setting to save on-device storage + LAN bytes.                                                                                | ✅ Done (via #49 Phase 1: `thumb_quality` setting + `defaultSettings`, pairs with paginated LAN fetch)                                                         |
| Q38 | **Haptic on queue-add**             | `navigator.vibrate(10)` after successful `queue_download` on Android.                                                                                           | ✅ Done (via centralized `api.queueDownload` + `vibrateTick` helper; covers all queue paths incl. batch)                                                       |
| Q39 | **Haptic helper (`vibrateTick`)**   | Central `vibrateTick(pattern)` helper reused for queue-add, long-press, pull-refresh.                                                                           | ✅ Done (`src/lib/haptics.ts` + `haptics.test.ts`; wired into browse queue-add (Q38), library long-press, `usePullToRefresh` engage)                           |
| Q40 | **Double-tap fullscreen (player)**  | Double-tap video toggles fullscreen on mobile player.                                                                                                           | ✅ Done (middle-third double-tap + action-bar Maximize/Minimize button, `playerContainerRef`, iOS `webkitEnterFullscreen` fallback)                            |
| Q41 | **Pull-to-refresh on Browse**       | Pull-to-refresh on browse list/detail pages, parity with library scenes grid.                                                                                   | ✅ Done (browse hub `reloadSites`/`loadTrending`/`refreshSaved` via `usePullToRefresh`; detail page already had it)                                            |
| Q42 | **Offline badge on cached scenes**  | "Offline" badge on `SceneCard` for on-device cached scenes.                                                                                                     | ✅ Done (auto badge on mobile local/standalone backends with file path + `offline` prop override; grid + list views)                                           |
| Q43 | **Storage row sizes**               | Settings → Storage row shows thumb-cache size + reclaimable orphan bytes.                                                                                       | ✅ Done (`thumb_cache_stats` command + `GET /api/library/thumb-cache` + Storage card with library/free/thumb/reclaimable + auto-scan on mount)                 |
| Q44 | **AMOLED quick toggle (header)**    | Header/theme-menu quick toggle for pure-black AMOLED variant.                                                                                                   | ✅ Done (`src/lib/amoled.ts` localStorage overlay + `.dark.amoled` CSS + sidebar Contrast button + Settings → Appearance AMOLED button)                        |
